...
- 2026-06-28 — **Iteration 18 bug-fix bundle** (4 fixes, 100% test pass):
   - **Image upload button works in Add mode** — the image modal was wrapped in `{isEdit && ...}` so it never rendered on /accounts/new. Modal now renders in both modes (Add stages locally, Edit uploads immediately). Accepted formats restricted to JPG/PNG/WebP/HEIC ≤10MB with explicit messaging.
   - **Single image viewer** — removed nested-fullscreen popup (`showFullscreen` state + jsx + Maximize2 import). Clicking the main image inside the viewer no longer spawns a second modal. Only Zoom In/Out/Reset + Prev/Next + Close remain.
   - **Opening Date locked + transaction-date validation** — frontend disables `opening-date` input in edit mode with "(locked after creation)" label. Backend rejects any `opening_date` change in PUT; validates every landed/received/close date is ≥ opening_date with a clear error.
   - **Receiving entry allocation (FIFO / Manual)** — rewrote `process_payment` to a **per-entry sequential** algorithm: clears entry-1 interest → entry-1 principal → entry-2 interest → entry-2 principal → … . Manual mode accepts `{landed_index, amount}[]` and applies each amount interest-first-then-principal within its entry. New `ReceivingAllocation` model + `allocation_method` / `allocations` fields on `ReceivedEntry`. New `POST /api/accounts/{id}/payments/preview` endpoint for live per-entry interest. One-time migration `migrate_allocations.py` replayed all existing accounts. New frontend pieces: `PaymentAllocationEditor` (embedded inline in both Add/Edit form and the new Record-Payment modal), `RecordPaymentModal` (launched from Account Detail). Received-entries tab now shows METHOD badge (FIFO / MANUAL) + "Applied To Landed Entries" per-row breakdown.
